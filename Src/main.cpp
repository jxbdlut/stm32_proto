...
int main(void)
{
  ...
  std::cout << "test!";
  uint8_t buffer[128];
  user_lucky lucky = user_lucky_init_zero;
  // int lucknumber[10] = {1, 2, 3, 4, 5, 6, 7, 8, 9};
  lucky.lucky_number = 1;
  pb_ostream_t stream_out = pb_ostream_from_buffer(buffer, sizeof(buffer));
  bool status = pb_encode(&stream_out, user_lucky_fields, &lucky);
  size_t msg_length = stream_out.bytes_written;
  ...
  user_lucky lucky2 = user_lucky_init_zero;
  pb_istream_t stream_in = pb_istream_from_buffer(buffer, msg_length);

  status = pb_decode(&stream_in, user_lucky_fields, &lucky2);
  /* Check for errors... */
  if (!status)
  {
